		...
		rl.question('Enter the code from that page here: ', code => {
			...
			auth.getToken(code, (err, token) => {
				...
				fs.writeFile(credentialsFile, JSON.stringify(token), err => {
					if (err) {
						reject(new Error(`Error writing ${credentialsFile} file:\n\t${err}`))
						return
					}
...
async function getAuth(clientSecretFile, credentialsFile) {
	...
	try {
		...
		throw new Error(`Error reading ${clientSecretFile} file:\n\t${err}`)
	}
...
async function getSpreadsheetRows(auth, spreadsheetId, range) {
	try {
		...
		throw new Error(`Error loading spreadsheet ${spreadsheetId} with range ${range}:\n\t${err}`)
 ...
 */
async function generateFiles(rows, keyIndex, langIndex, outputDir, format) {
	if (rows.length === 0) {
	...
	if (!fs.existsSync(outputDir)) {
		...
		try {
			...
			console.error(`Error writing directory ${outputDir}:\n\t${err}`)
		}
	}
...
module.exports = {
	...
	async generateFilesFromSpreadsheet(
		...
		try {
			...
			await generateFiles(rows, keyIndex, langIndex, outputDir, format)
		} catch (error) {
